...
void CPlateLocate::setLifemode(bool param) {
  if (param) {
    ...
    setMorphSizeWidth(10); //7
    setMorphSizeHeight(3);
    setVerifyError(0.75); //0.9
    setVerifyAspect(4.0);
    setVerifyMin(1);
    setVerifyMax(200);//30
  } else {
    setGaussianBlurSize(DEFAULT_GAUSSIANBLUR_SIZE);
    setMorphSizeWidth(DEFAULT_MORPH_SIZE_WIDTH);//17
    setMorphSizeHeight(DEFAULT_MORPH_SIZE_HEIGHT);
    setVerifyError(DEFAULT_ERROR);//0.6
    setVerifyAspect(DEFAULT_ASPECT);// 3.75
    setVerifyMin(DEFAULT_VERIFY_MIN);// 1
    setVerifyMax(DEFAULT_VERIFY_MAX);// 24
  }
}
...
bool CPlateLocate::verifySizes(RotatedRect mr) {
  ...
  int min = 34 * 8 * m_verifyMin;  // minimum area  44*14
...
int CPlateLocate::sobelOper(const Mat &in, Mat &out, int blurSize, int morphW,
  ...
  Mat mat_blur;
  mat_blur = in.clone();
  GaussianBlur(in, mat_blur, Size(blurSize, blurSize), 0, 0, BORDER_DEFAULT);//高斯模糊。Size中的数字影响车牌定位的效果。

  Mat mat_gray;
  if (mat_blur.channels() == 3)
    cvtColor(mat_blur, mat_gray, CV_RGB2GRAY);//为边缘检测算法准备灰度化环境
  else
    mat_gray = mat_blur;
...
//////Soble算子
  ...
  morphologyEx(mat_threshold, mat_threshold, MORPH_CLOSE, element);//二值化的图像进行闭操作
