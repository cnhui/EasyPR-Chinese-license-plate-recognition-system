...
namespace easypr {
  ...
  namespace demo {
    ...
    int  (const char* test_path, Result& result, bool useParams = false) {
...
#pragma omp critical
	...
	////////////////////////////////////////////////////////////////  
	  ...
	  ofstream myfile("resources/result/lookcar.txt", ios::app);
      if (myfile.is_open()) {
		...
		//matchPlate->getPlateStr()
		//string imageName = Utils::getFileName(filepath);
	    string imageName = Utils::getFileName(filepath);
        //img_ss << kv->get("original_image") << ":" << imageName << endl;
		myfile << kv->get("original_image") << ":" << imageName << endl;
		myfile << detectPlateLicense << endl;
