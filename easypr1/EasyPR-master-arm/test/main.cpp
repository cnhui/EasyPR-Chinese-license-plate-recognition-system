...
int testMain() {
  ...
  while (!isExit) {
    ...
    while (isRepeat) {
      ...
      switch (select) {
        case 1:
          assert(test_plate_locate() == 0);//    车牌定位  
          break;
        case 2:
          assert(test_plate_judge() == 0);// 车牌判断   
          break;
        case 3:
          assert(test_plate_detect() == 0);//车牌检测       
          break;
        case 4:
          assert(test_chars_segment() == 0);//  字符分隔             
          break;
        case 5:
          assert(test_chars_identify() == 0);//  字符鉴别     
          break;
        case 6:
          assert(test_chars_recognise() == 0);//字符识别       
          break;
        case 7:
          assert(test_plate_recognize() == 0);//   车牌识别  
          break;
        case 8:// 测试全部
...
//printf("6666\n");//gh
		  ...
		  FILE *pfile;  
		  if ((pfile = fopen("example.txt", "ab+")) == NULL)  
		   {  
			exit(0);  
		    } 
